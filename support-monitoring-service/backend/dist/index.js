...
app.put("/api/support/tickets/:id/resolve", async (req, res) => {
    ...
    try {
        const dbRes = await pool.query(`UPDATE support_tickets SET status = $1, resolution = $2, resolved_by = $3
       WHERE ticket_id = $4 OR id = $5 RETURNING *`, [status, resolution, resolvedBy, String(id), isNaN(Number(id)) ? -1 : Number(id)]);
