...
app.get(['/api/delivery/orders/active', '/api/delivery/orders'], async (_req, res) => {
  try {
    ...
    const formatted = (dbRes.rows || []).map(o => ({
      ...
      items: Array.isArray(o.item_details) ? o.item_details.map((i: any) => `${i.name} (${i.quantity})`) : ['Grocery Items'],
...
function findNearestWarehouse(custLat: number, custLng: number, warehouses: any[]): any {
    ...
    if (gatewayPgPool) {
      ...
      if (wRes.rows && wRes.rows.length > 0) {
        if (orderRow?.warehouse_id) {
          const matchedWh = wRes.rows.find((w: any) => Number(w.id) === Number(orderRow.warehouse_id));
...
app.post('/api/orders/:id/assign-rider', async (req, res) => {
  ...
  try {
    const dbRes = await gatewayPgPool.query(
      ...
      [riderId || null, riderName || 'Delivery Partner', riderPhone || '', targetId]
    );
    if (dbRes.rows && dbRes.rows.length > 0) {
