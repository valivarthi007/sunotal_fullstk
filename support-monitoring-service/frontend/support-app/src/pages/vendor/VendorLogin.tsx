...
export default function VendorLogin() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col justify-between selection:bg-emerald-500/20">
      {/* Minimal Dedicated Vendor Portal Header */}
      <header className="border-b bg-card py-4 px-6 shadow-2xs">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-lg leading-tight text-secondary">Sunotal Vendor Portal</h1>
              <p className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wider">Direct Farm Sourcing Engine</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="py-12 px-4 flex-1 flex flex-col justify-center items-center">
        <div className="w-full max-w-md bg-card border border-border shadow-xl rounded-3xl p-8 space-y-6">
          <div className="text-center space-y-2">
            ...
            <h2 className="text-2xl font-bold text-secondary">Vendor Sign In</h2>
            <p className="text-xs text-muted-foreground">Access crop harvest quotations, inventory & payout invoices</p>
          </div>
      ...
      <footer className="border-t bg-card py-4 text-center text-xs text-muted-foreground">
        Sunotal Direct Farm Sourcing & Vendor Portal &copy; {new Date().getFullYear()}
      </footer>
    </div>
  );
}
