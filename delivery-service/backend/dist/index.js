...
app.post('/api/delivery/orders/:id/accept', async (req, res) => {
    ...
    try {
        ...
        const orderLocRes = await client.query(`SELECT o.delivery_latitude, o.delivery_longitude, o.latitude, o.longitude, w.latitude as wh_lat, w.longitude as wh_lng
       FROM orders o
       LEFT JOIN warehouses w ON o.warehouse_id = w.id
       WHERE o.id::text = $1 OR o.order_number = $1 LIMIT 1`, [String(orderId)]).catch(() => ({ rows: [] }));
       ...
       RETURNING *`, [orderId, rId, rName, rPhone, currentLat || null, currentLng || null, destLat || null, destLng || null]);
